...
def Mutation1(instance: ProblemInstance, I_m: Solution) -> Solution:
    ...
    I_m = move_destination(instance, I_m, random_vehicle, origin_position, random_vehicle, destination_position)

    return I_m

def Mutation2(instance: ProblemInstance, I_m: Solution) -> Solution:
    random_vehicle = get_random_vehicle(I_m, vehicles_required=2)
    num_customers = I_m.vehicles[random_vehicle].getNumOfCustomersVisited()
    origin_position = rand(1, num_customers)
    ...
    for i in range(1, num_customers + 1):
        if not i == origin_position:
            I_m = move_destination(instance, I_m, random_vehicle, origin_position, random_vehicle, i)

            if 0 <= I_m.total_distance < fitness_of_best_location:
                fitness_of_best_location = I_m.total_distance
                best_location = i
            I_m = move_destination(instance, I_m, random_vehicle, i, random_vehicle, origin_position + 1 if origin_position > i else origin_position)

    if not best_location == origin_position:
        I_m = move_destination(instance, I_m, random_vehicle, origin_position, random_vehicle, best_location)

    return I_m

def Mutation3(instance: ProblemInstance, I_m: Solution) -> Solution:
    ...
    I_m = move_destination(instance, I_m, random_origin_vehicle, origin_position, random_destination_vehicle, destination_position)
...
def Mutation4(instance: ProblemInstance, I_m: Solution) -> Solution:
    random_origin_vehicle = get_random_vehicle(I_m)
    origin_position = rand(1, I_m.vehicles[random_origin_vehicle].getNumOfCustomersVisited())
    ...
    for i in range(1, I_m.vehicles[random_destination_vehicle].getNumOfCustomersVisited() + 1):
        I_m = move_destination(instance, I_m, random_origin_vehicle, origin_position, random_destination_vehicle, i)

        if 0 <= I_m.total_distance < fitness_of_best_location:
            fitness_of_best_location = I_m.total_distance
            best_location = i
        I_m = move_destination(instance, I_m, random_destination_vehicle, i, random_origin_vehicle, origin_position)

    if best_location >= 0:
        I_m = move_destination(instance, I_m, random_origin_vehicle, origin_position, random_destination_vehicle, best_location)
        if not I_m.vehicles[random_origin_vehicle].getNumOfCustomersVisited():
            del I_m.vehicles[random_origin_vehicle]

    return I_m

def Mutation5(instance: ProblemInstance, I_m: Solution) -> Solution:
    random_origin_vehicle = get_random_vehicle(I_m)
    origin_position = rand(1, I_m.vehicles[random_origin_vehicle].getNumOfCustomersVisited())
    ...
    I_m = move_destination(instance, I_m, random_origin_vehicle, origin_position, random_destination_vehicle, destination_position)
    I_m = move_destination(instance, I_m, random_destination_vehicle, destination_position + 1, random_origin_vehicle, origin_position)

    return I_m

def Mutation6(instance: ProblemInstance, I_m: Solution) -> Solution:
    random_origin_vehicle = get_random_vehicle(I_m)
    origin_position = rand(1, I_m.vehicles[random_origin_vehicle].getNumOfCustomersVisited())
    ...
    for destination_vehicle in range(0, len(I_m.vehicles)):
        if not random_origin_vehicle == destination_vehicle:
            ...
            if num_customers > 0:
                for i in range(1, num_customers + 1):
                    I_m = move_destination(instance, I_m, random_origin_vehicle, origin_position, destination_vehicle, i)

                    if 0 <= I_m.total_distance < fitness_of_best_location:
                        fitness_of_best_location = I_m.total_distance
                        best_vehicle = destination_vehicle
                        best_location = i
                    I_m = move_destination(instance, I_m, destination_vehicle, i, random_origin_vehicle, origin_position)

    if best_location >= 0:
        I_m = move_destination(instance, I_m, random_origin_vehicle, origin_position, best_vehicle, best_location)
        if not I_m.vehicles[random_origin_vehicle].getNumOfCustomersVisited():
            del I_m.vehicles[random_origin_vehicle]

    return I_m

def Mutation7(instance: ProblemInstance, I_m: Solution) -> Solution:
    random_origin_vehicle = get_random_vehicle(I_m)
    origin_position = rand(1, I_m.vehicles[random_origin_vehicle].getNumOfCustomersVisited())
...
def Mutation8(instance: ProblemInstance, I_m: Solution) -> Solution:
    if len(I_m.vehicles) < instance.amount_of_vehicles:
        ...
        I_m.objective_function(instance)

    return I_m

def Mutation9(instance: ProblemInstance, I_m: Solution) -> Solution:
    if len(I_m.vehicles) < instance.amount_of_vehicles:
        random_vehicle = get_random_vehicle(I_m, vehicles_required=2)
        num_customers = I_m.vehicles[random_vehicle].getNumOfCustomersVisited()
        origin_position = rand(1, num_customers)
...
def Mutation10(instance: ProblemInstance, I_m: Solution) -> Solution:
    random_origin_vehicle = get_random_vehicle(I_m)
        ...
        while destination_vehicle < len(I_m.vehicles) and not node_reallocated:
            ...
            if not random_origin_vehicle == destination_vehicle and num_customers_destination >= 1:
                for destination_position in range(1, num_customers_destination + 1):
                    I_m = move_destination(instance, I_m, random_origin_vehicle, origin_position, destination_vehicle, destination_position)

                    if I_m.total_distance > MMOEASA_INFINITY / 2:
                        I_m = move_destination(instance, I_m, destination_vehicle, destination_position, random_origin_vehicle, origin_position)
                    else:
