...
def calculate_median_Hypervolumes(ND_solutions: List[Union[MMOEASASolution, OmbukiSolution]], TWIH_Hypervolumes: Tuple[float, float, float]) -> float:
    prev_TD, prev_DU, prev_CU = ref_TD, ref_DU, ref_CU = TWIH_Hypervolumes[0], TWIH_Hypervolumes[1], TWIH_Hypervolumes[2]
    area = 0.0

    if isinstance(ND_solutions[0], MMOEASASolution):
